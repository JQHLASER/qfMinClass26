...
namespace qfWork
{
    internal class 编码_计算
    {
        private 编码 _BM_sys;
        public 编码_计算(编码 _BM_sys_)
        {
            this._BM_sys = _BM_sys_;
        }

        internal bool 计算(_BM_元素_文本_ info,  out string 结果, out string msgErr)
        {
            bool rt = true;
            结果 = info.内容;
            msgErr = string.Empty;
        ...
        internal bool 计算(_BM_班次_结构_[] info, DateTime 时间, out string 结果, out string msgErr)
        {
            bool rt = true;
            结果 = "";
            msgErr = string.Empty;
            try
            {
                ...
                {
                    _BM_班次_结构_ info班次S = new _BM_班次_结构_();
                    string 代码 = s.代码;
        ...
        /// </summary>     
        internal bool 计算(ref _BM_元素_序列号_ info, _BM_序列号操作_ 操作, DateTime 时间, DateTime 最后一次加工时间, _BM_班次_结构_[] 班次结构, out string 结果, out string msgErr)
        {
            ...
            if (!this._BM_sys._功能.序列号.每个加工数量 && info.加工.每个数量 != 1)
            {
                info.加工.每个数量 = 1;
                info.加工.当前计数 = 0;
            }
            ...
            try
            {
                ...
                switch (操作)
                {
                    ...
                    //判断复位
                    case _BM_序列号操作_.判断复位:
                        序列号复位(ref info, 最后一次加工时间, 时间, 班次结构, 序号fz);
                        序列号结果(ref info, 序号fz, out 结果);
                        break;

                    //强制复位
                    case _BM_序列号操作_.强制复位:
                    ...
                    //强制递减
                    case _BM_序列号操作_.强制递减:
                        // 序列号复位(ref info, 文件, 时间, 班次结构, 序号结构);
                        序列号强制递减(ref 序号fz);
                        序列号结果(ref info, 序号fz, out 结果);
                        break;
                    case _BM_序列号操作_.加工数量递增:
                        序列号复位(ref info, 最后一次加工时间, 时间, 班次结构, 序号fz);
                        序列号结果(ref info, 序号fz, out 结果);
        ...
        internal bool 计算(_BM_元素_日期_ info, DateTime 时间, out string 结果, out string msgErr)
        {
            bool rt = true;
            msgErr = string.Empty;
            结果 = "";
            _BM_日期时间编码类型_ 编码类型 = _BM_日期时间编码类型_.年4位;
            try
            {
                ...
                switch (info.类型)
                {
                    case _BM_日期_.年4位:
                        ...
                        编码类型 = _BM_日期时间编码类型_.年4位;
                        break;
                    case _BM_日期_.年2位:
                        结果 = nows.ToString("yy").ToString();
                        编码类型 = _BM_日期时间编码类型_.年2位;
                        break;
                    case _BM_日期_.月:
                        结果 = nows.ToString("MM").ToString();
                        编码类型 = _BM_日期时间编码类型_.月;
                        break;
                    case _BM_日期_.日:
                        结果 = nows.ToString("dd").ToString();
                        编码类型 = _BM_日期时间编码类型_.日;
                        break;
                    case _BM_日期_.天:
                    ...
                    case _BM_日期_.星期:
                        ...
                        编码类型 = _BM_日期时间编码类型_.星期;
                        break;
                    case _BM_日期_.周:
                        结果 = new qfmain.日期时间_().Get_weeks(nows).ToString("00");
                        编码类型 = _BM_日期时间编码类型_.周;
                        break;
                }

                结果 = this._BM_sys._ac_文件.Get_日期时间编码(编码类型, info.编码文件, 结果);
        ...
        internal bool 计算(_BM_元素_时间_ info, DateTime 时间, out string 结果, out string msgErr)
        {
            bool rt = true;
            msgErr = string.Empty;
            结果 = "";
            _BM_日期时间编码类型_ 编码类型 = _BM_日期时间编码类型_.时;
            try
            {
                switch (info.类型)
                {
                    ...
                    case _BM_时间_.分:
                        ...
                        编码类型 = _BM_日期时间编码类型_.分;
                        break;
                    case _BM_时间_.秒:
                        结果 = 时间.ToString("ss").ToString();
                        编码类型 = _BM_日期时间编码类型_.秒;
                        break;
                    case _BM_时间_.毫秒:
        ...
        /// <param name="结果"></param>
        /// <returns></returns>
        internal bool 计算(_BM_元素_关联对象_ info, string 对象名, string 关联对象的内容, out string 结果, out string msgErr)
        {
            msgErr = string.Empty;
            结果 = string.Empty;
                ...
                switch (info.类型)
                {
                    case _BM_关联类型_.按位:
                        ...
                        try
                        {
                            ...
                            _BM_关联对象_按位_ info按位 = new 编码_json().json_解析<_BM_关联对象_按位_>(info.参数);
                            ...
                            else
                            {
                                ...
                                {
                                    结果 = new 文本().获取(数据, 开始位, 数据.Length - 开始位);
                                }
                                else
                                {
                                    结果 = new 文本().获取(数据, 开始位, (int)info按位.数量);
                                }
                            }
                    ...
                    case _BM_关联类型_.按字符分割:

                        #region 按字符分割

                        try
                        {
                            _BM_关联对象_按字符分割_  info按字符 = new 编码_json().json_解析<_BM_关联对象_按字符分割_>(info.参数);
                        ...
                        break;
                    case _BM_关联类型_.按首尾字符:

                        #region 按开始和结束字符

                        try
                        {
                         _BM_关联对象_按首尾字符_  info_2 = new 编码_json().json_解析<_BM_关联对象_按首尾字符_>(info.参数);                      
                            string[] data取两个之符之间 = new 文本().获取(数据, info_2.首字符, info_2.尾字符);
        ...
        void 序列号_转换成可计算值(_BM_元素_序列号_ info, out _fz_序列号_ cfgFZ)
        {
            cfgFZ = new _fz_序列号_();
            cfgFZ.递增量 = info.递增量;
            ...
            //else if (info.类型 == _BM_序列号类型_.三十六进制)
            //{
        ...
        void 序列号_转换成最终值(ref _BM_元素_序列号_ info, _fz_序列号_ cfgFZ)
        {
            string 结果 = info.当前序号;
        ...
        /// <param name="info"></param>
        /// <returns></returns>
        internal int 序列号判断复位计算(DateTime 时间, DateTime 最后一次加工时间, _BM_班次_结构_[] 班次, _BM_元素_序列号_ info, _fz_序列号_ cfgFz)
        {
            int rt = 0;
        ...
        /// <param name="info"></param>
        /// <returns></returns>
        internal int 判断_序列号_加工计数复位(_BM_元素_序列号_ info)
        {
            int rt = 0;
        ...
        void 序列号复位(ref _BM_元素_序列号_ info, DateTime 时间, DateTime 最后一次加工时间, _BM_班次_结构_[] 班次结构, _fz_序列号_ 序号结构)
        {
        ...
        void 序列号结果(ref _BM_元素_序列号_ info, _fz_序列号_ 序号结构, out string 结果)
        {
        ...
        DateTime 日期时间_偏移计算(DateTime 时间, _BM_元素_日期_ info)
        {
            DateTime 当前时间 = 时间;
            if (this._BM_sys._功能.日期时间.自定义编码)
            {
                switch (info.偏移类型)
                {
                    case _BM_日期偏移类型_.无:
