...
namespace qf_Laser
{
    public partial class ui_bitmap_jcz多头 : UserControl
    {
        ...
        /// 日志控件
        /// </summary>
        public qfNet.ui_Log ui_log = new qfNet.ui_Log()
        {
        ...
        async void On_刷新图像(int cardindex, _激光_获取图像_ state)
        {
        ...
        async Task<bool> getBitmap(int cardindex, _激光_获取图像_ state)
        {
            try
            {
                Task t0 = Task.Run(() =>
        ...
        void On_加载Ezd(int Cardindex, string ezdPath, _Err_jczMarkEzd2_ rt)
        {
            ...
            {
                this.Add(false, $"{qfmain.Language_.Get语言("加载激光模板")},{JczLmc_Multiline.ErrMsg(rt)},{this._markEzd._lst_参数[this._CardIndex]._Path_ezd}");
            }
