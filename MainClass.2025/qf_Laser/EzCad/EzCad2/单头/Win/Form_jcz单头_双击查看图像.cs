...
namespace qf_Laser
{
    public partial class Form_jcz单头_双击查看图像 : Sunny.UI.UIForm
    {
        ...
        public Form_jcz单头_双击查看图像(MarkEzd_Ezd2 markEzd_, MultilineMarkEzd multilineMarkEzd_ = null, int CardIndex = -1)
        {
            InitializeComponent();
        ...
        private void Form_jcz单头_双击查看图像_Load(object sender, EventArgs e)
        {
            if (this._markEzd != null)
            {
                this._markEzd.刷新图形(_激光_获取图像_.获取);
            }
            else if (this._multilineMarkEzd != null)
            {
                this._multilineMarkEzd.刷新图形(this._CardIndex, _激光_获取图像_.获取);
            }
        }
        ...
        async void On_获取图像_multiline(int Cardindex, _激光_获取图像_ state)
        {
            var t0 = await getBitmap_multiline(Cardindex, state);
        }
        async Task<bool> getBitmap_multiline(int cardindex, _激光_获取图像_ state)
        {
