...
namespace qfmain
{
    ...
    public class SqlSugar_DB : IDisposable
    {
        ...
        void 初始化(_info_SQLite_ info, int 超时时间 = 1000 * 10)
        {
        ...
        void 初始化(_info_SQLserver_ info, enumSQLserver 驱动 = enumSQLserver.新驱动, int 超时时间 = 1000 * 10)
        {
        ...
        void 初始化(_info_Oracle_ info, int 超时时间 = 1000 * 10)
        {
            string connectStr = 数据库连接字符中(info);
            Db = new SqlSugarScope(new ConnectionConfig()
            {
        ...
        void 初始化(_info_Mysql_ info, int 超时时间 = 1000 * 10)
        {
