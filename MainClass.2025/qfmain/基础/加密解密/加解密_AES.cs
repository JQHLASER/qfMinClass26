...
{
    /// <summary>
    /// 对称加密使用相同的密钥进行加密和解密，效率高，适合处理字符串等数据。
    /// </summary>
    public class 加解密_AES
    {

        // 密钥和向量需妥善保管（长度：AES密钥16/24/32字节，向量16字节）
        private readonly byte[] Key = Encoding.UTF8.GetBytes("1234567890123456"); // 16字节
        private readonly byte[] Iv = Encoding.UTF8.GetBytes("abcdefghijklmnop");  // 16字节

        // 加密字符串
        public virtual string 加密(string plainText)
        {
            using (Aes aesAlg = Aes.Create())
            {
                ...
                using (MemoryStream msEncrypt = new MemoryStream())
                {
                    using (CryptoStream csEncrypt = new CryptoStream(msEncrypt, encryptor, CryptoStreamMode.Write))
                    {
                        ...
                        return Convert.ToBase64String(encryptedBytes); // 转为Base64方便传输
                    }
                }
            }
        }
        ...
        // 解密字符串
        public virtual string 解密(string cipherText)
        {
            ...
            using (Aes aesAlg = Aes.Create())
            {
                ...
                using (MemoryStream msDecrypt = new MemoryStream(cipherBytes))
                {
                    using (CryptoStream csDecrypt = new CryptoStream(msDecrypt, decryptor, CryptoStreamMode.Read))
                    {
                        ...
                        {
                            return srDecrypt.ReadToEnd(); // 读取解密后的明文
                        }
                    }
                }
            }
      ...
      0x3F
        ...
        public virtual string 加密_1(string encryptString, string encryptKey, Encoding encoding = null)
        {
            if (encoding == null)
            {
        ...
        public virtual string 解密_1(string decryptString, string decryptKey, Encoding encoding = null)
        {
            try
            {
                if (encoding == null)
        ...
        /// <returns>加密流</returns>
        public virtual CryptoStream 加密文件流(FileStream fs, string decryptKey, Encoding encoding = null)
        {
            if (encoding == null)
            {
                encoding = Encoding.UTF8;
        ...
        /// <returns>加密流</returns>
        public virtual CryptoStream 解密文件流(FileStream fs, string decryptKey, Encoding encoding = null)
        {
            if (encoding == null)
            {
                encoding = Encoding.UTF8;
        ...
        public virtual void 加密流到文件(FileStream input, string outputPath)
        {
            using (FileStream fren = new FileStream(outputPath, FileMode.Create))
            {
                CryptoStream enfr = 加密文件流(fren, Default_AES_Key);
        ...
        /// <param name="input">源文件流</param>
        /// <param name="outputPath">输出文件路径</param>
        public virtual void 解密流到文件(FileStream input, string outputPath)
        {
            FileStream frde = new FileStream(outputPath, FileMode.Create);
            CryptoStream defr =  解密文件流(input, Default_AES_Key);
