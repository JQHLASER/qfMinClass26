...
namespace qfmain 
{
    ...
    internal struct SP_INTERFACE_DEVICE_DATA
    {
        public int cbSize;
    ...
    internal struct SP_INTERFACE_DEVICE_DATA_64
    {
        public Int64 cbSize;
        public GUID InterfaceClassGuid;
        public int Flags;
    ...
    internal class 天威
    {
        ...
        /// <returns></returns>
        public int 获取_锁的版本(ref short Version, string Path)
        {
            int ret;
            int hsignal;
        ...
        /// <summary>
        /// 获取加密狗ID,id=id_1.ToString("X8")+id_2.ToString("X8")
        /// </summary>
        /// <param name="ID_1"></param>
        /// <param name="ID_2"></param>
        /// <param name="Path"></param>
        /// <returns></returns>
        public int 获取_锁ID(ref int ID_1, ref int ID_2, string Path)
        {
            int ret;
            int hsignal;
            hsignal = CreateSemaphore(0, 1, 1, "ex_sim");
        ...
        /// <param name="Path"></param>
        /// <returns></returns>
        public int 设置_读锁密码(string W_HKey, string W_LKey, string new_HKey, string new_LKey, string Path)
        {
            int ret;
            int hsignal;
            byte[] ary1 = new byte[8];
        ...
        //设置写入密码
        public int 设置_写锁密码(string W_HKey, string W_LKey, string new_HKey, string new_LKey, string Path)
        {
            int ret;
            int hsignal;
            byte[] ary1 = new byte[8];
        ...
        /// <param name="Path"></param>
        /// <returns></returns>
        public int 写入_字符串(string InString, int address, string HKey, string LKey, string Path)
        {
            int ret = 0;
            byte[] ary1 = new byte[8];
            int hsignal;
        ...
        /// <param name="address">开始地址,锁起始地址是0</param>
        /// <param name="nlen">字符串长度</param>
        /// <param name="HKey"></param>
        /// <param name="LKey"></param>
        /// <param name="Path"></param>
        /// <returns></returns>
        public int 获取_字符串(ref string OutString, int address, int nlen, string HKey, string LKey, string Path)
        {
            int ret = 0;
            byte[] ary1 = new byte[8];
            int hsignal;
        ...
        /// <param name="Path"></param>
        /// <returns></returns>
        public int 恢复出厂设置_初始化(string Path)
        {
            int ret;
            int hsignal;
            hsignal = CreateSemaphore(0, 1, 1, "ex_sim");
        ...
        /// 设置iKey不显示盘符
        /// </summary>
        /// <param name="IsHidOnly"></param>
        /// <param name="Path"></param>
        /// <returns></returns>
        public int 设置_ikey不显示盘符(bool IsHidOnly, string Path)
        {
            int ret;
            int hsignal;
            hsignal = CreateSemaphore(0, 1, 1, "ex_sim");
        ...
        /// <param name="Path"></param>
        /// <returns></returns>
        public int 设置_iKey为只读模式(string Path)
        {
            int ret;
            int hsignal;
            hsignal = CreateSemaphore(0, 1, 1, "ex_sim");
        ...
        public bool 获取_字符串_任意长度(string KeyPath, short StartAddress, string HKey, string LKey, ref string value)
        {
            ...
            //short addr = 0;//要读取的地址
            short addr = StartAddress;

            //先从地址0读到以前写入的字符串的长度
            ret = YReadEx(buf, addr, 1, HKey, LKey, KeyPath);
            nlen = buf[0];
            if (ret != 0)
            {
            ...
            ret = 获取_字符串(ref outstring, addr + 1, nlen, HKey, LKey, KeyPath);
            if (ret != 0)
            {
            ...
            {
                //  MessageBox.Show("已成功读取字符串：" + outstring.ToString());
                value = outstring;
                return true;
            }
        ...
        public bool 写入_字符串_任意长度(string KeyPath, short StartAddress, string HKey, string LKey, string value)
        {
            ...
            byte[] buf = new byte[1];
            // int addr = 0;//要写入的地址
            int addr = StartAddress;//要写入的地址


            InString = value;
            nlen = lstrlenA(InString);
            buf[0] = System.Convert.ToByte(nlen);

            //写入字符串到地址1
            ret = 写入_字符串(InString, addr + 1, HKey, LKey, KeyPath);
            if (ret != 0)
            {
            ...
            //写入字符串的长度到地址0
            ret = YWriteEx(buf, addr, 1, HKey, LKey, KeyPath);
            if (ret != 0)
            {
                // MessageBox.Show("写入字符串长度错误。错误码：" + ret.ToString());
                return false;
            }
            else
            {
                //MessageBox.Show("写入字符串成功");
                return true;
            }
        }
    ...
    internal class DogTWclass
    {
        ...
        /// <param name="版本"></param>
        /// <returns></returns>
        public static int 获取_狗版本(out short 版本)
        {
            short value = 0;
            int rt = TW.获取_锁的版本(ref value, "");
            版本 = value;
            return rt;
        }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public static int 获取_狗id(out string id)
        {
            int id1 = 0, id2 = 0;
            int rt = TW.获取_锁ID(ref id1, ref id2, "");
            id = id1.ToString("X8") + id2.ToString("X8");
            return rt;
        }
        ...
        /// 
        /// </summary>
        /// <param name="addressStart">开始地址</param>
        /// <param name="Hkey"></param>
        /// <param name="Lkey"></param>
        /// <param name="value"></param>
        /// <returns></returns>
        public static bool 获取_字符串_任意长度(short addressStart, string Hkey, string Lkey, ref string value)
        {
            return TW.获取_字符串_任意长度("", addressStart, Hkey, Lkey, ref value);
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static bool 写入_字符串_任意长度(short addressStart, string Hkey, string Lkey, string value)
        {
            return TW.写入_字符串_任意长度("", addressStart, Hkey, Lkey, value);
        ...
        /// </summary>
        /// <param name="addressStart">开始地址</param>
        /// <param name="accress数量">地址数量</param>
        /// <param name="Hkey"></param>
        /// <param name="Lkey"></param>
        /// <param name="value"></param>
        /// <returns></returns>
        public static int 获取_字符串(int addressStart, int accress数量, string Hkey, string Lkey, ref string value)
        {
            return TW.获取_字符串(ref value, addressStart, accress数量, Hkey, Lkey, "");
        }
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static int 写入_字符串(int addressStart, string Hkey, string Lkey, string value)
        {
            return TW.写入_字符串(value, addressStart, Hkey, Lkey, "");
        }


        public static int 设置_恢复出厂设置()
        {
            return TW.恢复出厂设置_初始化("");
        }


        /// <summary>
        /// 返回,0:正常, 没啥用,反正都不显示盘符
        /// </summary>
        /// <param name="TshowFnoshow">true:显示,false:不显示</param>
        /// <returns></returns>
        public static int 设置_ikey是否显示盘符(bool TshowFnoshow)
        {
            return TW.设置_ikey不显示盘符(TshowFnoshow, "");
        }

        public static int 设置_iKey为只读模式()
        {
            return TW.设置_iKey为只读模式("");
        }

        /// <summary>
        /// 返回,0:正常
        /// </summary>
        /// <param name="H_key_旧"></param>
        /// <param name="L_key_旧"></param>
        /// <param name="H_key_新"></param>
        /// <param name="L_key_新"></param>
        /// <returns></returns>
        public static int 设置_写锁密码(string H_key_旧, string L_key_旧, string H_key_新, string L_key_新)
        {
            return TW.设置_写锁密码(H_key_旧, L_key_旧, H_key_新, L_key_新, "");
        }
        ...
        /// <param name="L_key_新"></param>
        /// <returns></returns>
        public static int 设置_读锁密码(string H_key_旧, string L_key_旧, string H_key_新, string L_key_新)
        {
            return TW.设置_读锁密码(H_key_旧, L_key_旧, H_key_新, L_key_新, "");
        }
