...
namespace qfmain
{
    public class 生产计数
    {
        ...
        public virtual  void 读写信息(ushort model)
        {
            ...
            _cfg_生产计数_ info = _当前计数信息;
            new qfmain.文件_文件夹().WriteReadIni(path, model, ref info, out string msgErr);
            _当前计数信息 = info;

            _当前计数信息.零件 = _使能_零件 ? _当前计数信息.零件 : 0;

        }

          
        public  void 计数递增_良品(int 递增量 = 1)
        {
            if (_使能_良品计数)
            {
                _当前计数信息.良品 += 递增量;
        ...
        public void 计数递增_不良品(int 递增量 = 1)
        {
            if (_使能_不良品计数)
            {
                _当前计数信息.不良品 += 递增量;
        ...
        public void 设置(long 零件, long 良品, long 不良品)
        {
            _当前计数信息.零件 = _使能_零件 ? 零件 : _当前计数信息.零件;
            _当前计数信息.良品 = _使能_良品计数 ? 良品 : _当前计数信息.良品;
            _当前计数信息.不良品 = _使能_不良品计数 ? 不良品 : _当前计数信息.不良品;
            读写信息(0);
